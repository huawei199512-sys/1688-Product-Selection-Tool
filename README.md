...
- **免费试用额度**：未登录（或登录但未配置自己 key）的访客按 IP 最多请求 50 次，超出后提示注册登录并配置 key/秘钥
- **管理员后台**：`/admin` 查看所有账号、各自 key 与收货地址（姓名/电话/地址已脱敏打码），以及按天的访问统计（哪个 IP 访问了什么）
...
| `FREE_REQUEST_LIMIT` | 每个 IP 的免费试用次数上限，默认 `50` |
